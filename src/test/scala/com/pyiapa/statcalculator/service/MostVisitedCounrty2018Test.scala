...
/**
 * Class to test functionality of MostVisitedCountry2018
 */
class MostVisitedCounrty2018Test extends FunSuite {
  
  final val MOST_VISITED_COUNTRY_2018_EXPECTED_RESULT = "Italy"
  
  val spark = MainSparkSession.getCurrentSparkSession()
  import spark.implicits._
  
  test("Test most visited Country in 2018") {
    
    val testDF = createNormalTestDataFrame()
    
    val countryDF = new MostVisitedCountry2018(testDF)
    
    val mostVisitedCountry2018 = countryDF.calculate()
    
    val result = mostVisitedCountry2018.result.select("location").first().get(0)
    
    assert(result === MOST_VISITED_COUNTRY_2018_EXPECTED_RESULT)
